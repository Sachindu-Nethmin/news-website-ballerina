...
const Page = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async ({item, customer, quantity}, helpers) => {
      try {
        const response = await postAPI(submitOrderUrl, { date: createdDate(), item, customerId: customer, status: 'PENDING', cargoId: "Not Assigned", id: createID(), quantity: quantity });
        if (response.error) {
          setError(true);
